    ...
    while(pointsToSpend > 0) {
        for (const transaction of transactionList) {
            if(transaction.points >= pointsToSpend){
                ...
                changes.push({payer: transaction.payer, points: 0 - transaction.points})
...
router.put('/', async (req, res) => {
    try {
        ...
        const [updatedList, changes] = spendPoints(pointsToSpend, orderedTransactions)
        
        for(const updateTransaction of updatedList) {
            await Transaction.update({points: updateTransaction.points},{
                where: {
